...
# Overlay the freshly built domain hooks bundle (generated by `bun run build`
# in stage 1) so the image never ships a stale copy from the build context.
COPY --from=frontend-builder /app/pb/pb_hooks/domain.js /app/pb_hooks/domain.js
